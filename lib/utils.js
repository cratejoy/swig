...
exports.each = function (obj, fn) {
  ...
  if(obj.is_tuple) {
    ...
    l = obj.length;
    for (i; i < l; i += 1) {
      if (fn(obj[i].slice(1), obj[i][0], obj) === false) {
        break;
      }
    }
  } else if (isArray(obj)) {
...
/**
 * Substitute reserved keywords with a prefixed one.
 */
exports.fixReserved = function(name) {
  var _reserved = ['break', 'case', 'catch', 'class', 'continue', 'debugger', 'default', 'delete', 'do', 'else', 'finally', 'for', 'function', 'if', 'in', 'instanceof', 'new', 'return', 'switch', 'this', 'throw', 'try', 'typeof', 'var', 'void', 'while', 'with'];
  if(_reserved.indexOf(name) !== -1) {
    return '_rsv_' + name;
  }
  return name;
}
