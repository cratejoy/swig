...
TokenParser.prototype = {
  ...
  scope_out: function(dontCount) {
    if(this.scope !== undefined && this.scope === this.fdivScope) {
      this.out.push(')');
      this.state.pop();
      this.filterApplyIdx.pop();
      delete this.fdivScope;
    }
    if(!dontCount) {
      this.scope--;
    }
  ...
  parseToken: function (token) {
	...
	*/
    ...
    case _t.STRING:
      self.kwVar = utils.fixReserved(match.replace(/["']/g, ''));
      self.filterApplyIdx.push(self.out.length);
      self.out.push(match.replace(/\\(?!"|n|\')/g, '\\\\'));
      break;
    ...
    case _t.FLOORDIV:
      self.state.push(_t.FILTER);
      self.out.splice(self.filterApplyIdx[self.filterApplyIdx.length - 1], 0, '_filters["floordiv"](');
      self.fdivScope = self.scope; // If we leave this scope (or comma in same scope), add a ')'
      token.type = _t.FILTER;
      break;

    case _t.INLINE_IF:
      for(var i=1; i<=3; i++) { 
        var vars = utils.filter(lexer.read(token.full[i]), function(t) { return t.type === _t.VAR; });
        utils.each(vars, function(v){
            token.full[i] = token.full[i].replace(
              v.match, 
              '((_ctx.' + v.match + ' === undefined) ? ' + v.match + ' : _ctx.' + v.match + ')');
        });
      }
	...
	case _t.ASSIGNMENT:
		if(lastState === _t.FUNCTION || lastState == _t.METHODOPEN || lastState == _t.FILTER || lastState == _t.KWDICT) {
			self.state.push(_t.KWARG);
			self.out.pop();

			// pop any prev comma.
			if(self.out.length && self.out[self.out.length - 1].trim() === ',') {
				self.out.pop();
			} else {
				self.ignoreNextCommaAtScope = self.scope;
			}

			var kwst = {
				key: self.kwVar,
				start: self.out.length
			};
			self.kwstate.push(kwst);
		}
		break;
    ...
    case _t.PARENOPEN:
      ...
      if(prevToken && prevTokenType === _t.VAR) {
        ...
        self.out.push('.call(' + self.checkMatch(temp));
        self.out.push(',');
    ...
    case _t.PARENCLOSE:
      ...
      if (temp === _t.KWARG) {
        temp = self.state.pop();
        kwst = self.kwstate.pop();
        self.kwargs[kwst.key] = self.out.splice(kwst.start).join('');
      }
    ...
    case _t.COMMA:
      ...
      if(self.ignoreNextCommaAtScope !== undefined && self.ignoreNextCommaAtScope === self.scope) {
        delete self.ignoreNextCommaAtScope;
      } else {
        self.out.push(', ');
        self.filterApplyIdx.pop();
      }
    ...
    case _t.VAR:
      ...
      self.kwVar = utils.fixReserved(match);
      break;
    ...
    case _t.KWDICT:
      self.state.push(_t.KWDICT);
      break;

    case _t.CURLYOPEN:
      self.state.push(_t.CURLYOPEN);
      self.out.push('{');
      self.filterApplyIdx.push(self.out.length - 1);
      self.scope_in();
      break;
    ...
    case _t.COLON:
      if (lastState !== _t.CURLYOPEN && lastState !== _t.KWDICT) {
        utils.throwError('Unexpected colon', self.line, self.filename);
      }
      if(lastState === _t.KWDICT) {
        token.type = _t.ASSIGNMENT;
        token.match = '=';
        self.parseToken(token);
        break;
      } else {
    ...
    case _t.CURLYCLOSE:
      if (lastState === _t.COLON || lastState == _t.KWARG) {
        self.state.pop();
      }
      ...
      var temp = self.state.pop();
      if (temp !== _t.CURLYOPEN && temp !== _t.KWDICT) {
  ...
  parseVar: function (token, match, lastState) {
    ...
    for(var i=0; i<match.length; i++){
      match[i] = utils.fixReserved(match[i]);
    }
...
exports.parse = function (swig, source, opts, tags, filters) {
  ...
  function parseVariable(str, line) {
	...
	// Check if this is our settings object.
	if(str.match(/^settings(\.|\[)/)) {
		// Extract only the variable part of the variable. ie, ignore any 
		// filter or operation that follows the variable.
		tokens.every(function(t) {
			switch(t.type) {
				case 0:
				case 9:	 _str.push(_brd ? '' : t.match);	break;
				case 12: _str.push(t.match); _brd++;		break;
				case 13: _str.push(t.match); _brd--;		break;
				default:
					return !!_brd;
			}
			return true;
		...
		// Concatenate the parts and remove the 'settings' part.
		_str = _str.join('').replace(/^settings/, '');

		// Extract anything inside square brackets, ex. foo.bar[...]
		_brd = 0;
		prs.forEach(function(p) {
			switch(p) {
				case '[':	_brd++ || (_tmp = []);				break;
				case ']':	--_brd || _evl.push(_tmp.join(''));	break;
				default:	_tmp.push(p);						break;
			}
		});
	
		// Yield code to replace square bracket contents with their evaluated
		// results and pass that new sting to our callback. Ex.
		// foo.bar[1+2] => foo.bar['3'].
		// NOTE: Keys are always used as strings.
		_pre += 'var _str = "{}";'.format(_str);
		_pre += 'var _evl = [];';
		_evl.forEach(function(e){ _pre += '_evl.push({});\n'.format(e); });
		_pre += '_str = _str.replace(/\\[\\]/g, function() { return "[\'" + _evl.shift() + "\']";});';
		_pre += 'try{if(eval("_ctx.settings" + _str + " === undefined")){_ctx.on_undefined_settings(_str);' +
		'}}catch(e){_ctx.on_undefined_settings(_str);}\n';
	}
	...
	if(parser.kwargs != undefined) {
		...
		_pre += '_filter_ctx = _ctx;\n';
		_post = 'delete _ctx.kwargs;\n';
		delete parser['kwargs'];
	}
	
	// Break into the debugger if you do {{ debugger }} in a template.
    if(str === 'debugger') {
      _pre = 'debugger;\n';
      _post = '';
      _out = '""';
